...
function SP(A,B,C,D,E,F,G,H,Ia,Ib,J,K,L,M,N,O,P,X) {
	...
	//SA = ((AA+B)*(2.7*G-1) + (2*D+(J*0.5+0.5))*(F/2+0.5)) / H
    //sp = C*SA / (AA*((K**2)/2+0.5)); 
    SA = ( (A+B)*(3*G-1) + (2*D+(J*0.5+0.5))* (F*2 -1) ) * C
    
    //SA = ( (A+B)*(2.7*G-1) + (2*D+(J*0.5+0.5))) * (F*0.5+0.5)  * C
...
function IP(A,B,C,D,E,F,G,H,Ia,Ib,J,K,L,M,N,O,P,X) {
	...
	if (B==1){ //創
		...
		IC =( 0.4*G + 2*N ) * ( A*1.5 ) * (((Ia**2)/2+0.5)) * (L/2+0.5);
		ID = ( 1/5*F +3/5 ) * ( P * (0.4*J+0.6) ) * (0.25*M);
...
function COST(A,B,C,D,E,F,G,H,Ia,Ib,J,K,L,M,N,O,P,X) {
	...
	//CB = (F*0.3+0.7)*2+O / ((K**2)*(N*0.25+0.75))
	//CC = J*(L*0.4+0.6)/Ia // I -> Ia
	
	
	CA = G*((D*0.25+0.75) +(B*0.75+0.25)*C) * ((F*0.3+0.7)*2+O) *(J*0.5+0.5)*(L*0.4+0.6)
	...
	//CA = (AA*0.5+0.5)*(D*0.25+0.75) +(B*0.75+0.25)*C / (H**2)
	//CB = (F*0.3+0.7)*2+O / ((K**2)*(N*0.25+0.75))
	//CC = (J*0.2+0.8)*(L*0.4+0.6)/Ia 
	
	CA = G*((D*0.25+0.75) +(B*0.75+0.25)*C) * ((F*0.3+0.7)*2+O) *(J*0.2+0.8)*(L*0.4+0.6)
	CB = (A*0.5+0.5) *  (H**2) * ((K**2)*(N*0.25+0.75)) * Ib
